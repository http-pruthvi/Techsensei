...
export const searchKnowledge = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
    if (!context?.auth) {
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            for (const term of searchTerms) {
                ...
                if (item.tags?.some((t: string) => t.toLowerCase().includes(term))) score += 3;
...
export const saveKnowledgeItem = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
    if (!context?.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
